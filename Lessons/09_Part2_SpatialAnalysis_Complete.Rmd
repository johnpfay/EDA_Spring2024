...
#-or-
ggplot(data=huc8_sf)+
  geom_sf(aes(fill=ACRES),alpha=0.5)+
...
#Bonus map
mapview(huc8_sf,col.regions=c('orange')) +
  mapview(upper_neuse,col.regions=c('purple'))
...
ggplot(data=huc8_sf_join) +
  geom_sf(aes(fill=SumWindEnergy),alpha=0.3) +
  labs(
...
ggplot(data=state_sf) + geom_sf(color='blue',fill='lightblue')
```
...
#Map the data
mapview(huc2_sf, zcol='ACRES',legend=F)
...
the_triangle <- counties_sf_utm %>%
  filter(NAME %in% c('Durham','Orange','Wake'))
