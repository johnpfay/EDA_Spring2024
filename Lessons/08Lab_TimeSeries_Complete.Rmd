...
Rainfall_BR_processed <-
  ...
  separate_wider_delim(Statistics,names=c("Null","Month","Null2"),delim = " ") %>%
...
temperature_df <- as.data.frame(temperature_decomposed$time.series[,1:3])%>% 
   mutate(
...
rain_data_trend2 <- trend::smk.test(rainfall_ts)
# Inspect results
rain_data_trend2
...
#Subtract seasonal from trend + remainder
rainfall_df$nonseasonal <- rainfall_df$trend + rainfall_df$remainder - rainfall_df$seasonal 

#Create a new time series object
rainfall_ts_adj <- ts(rainfall_df$nonseasonal,
                   start=c(f_year,f_month),
                   frequency=12) 

#Decompose
rainfall_stl_adj <- stl(rainfall_ts_adj,s.window = "periodic")
plot(rainfall_stl_adj)



#Test - lm
rain1 <- lm(data=rainfall_df, nonseasonal ~ Date)
summary(rain1)

#Test - Mann-Kendall
rain2 <- Kendall::MannKendall(rainfall_ts_adj)
summary(rain2)
